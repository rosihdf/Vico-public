...
| Ursache | Lösung |
|---------|--------|
| **Haupt-App und Portal nutzen unterschiedliche Supabase** | Beide müssen dieselbe Datenbank nutzen. Prüfen: `VITE_SUPABASE_URL` und `VITE_SUPABASE_ANON_KEY` in Haupt-App und `arbeitszeit-portal/.env` müssen identisch sein (gleiches Supabase-Projekt wie die Zeiterfassung). Siehe `docs/Netlify-Vier-Apps.md` (vier Apps, gleiche DB für Zeiterfassung). |
