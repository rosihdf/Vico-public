...
import { setStoredLicenseNumber, fetchLicenseFromApi } from '../lib/licensePortalApi'
...
const AktivierungsScreen = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError('Bitte Lizenznummer eingeben.')
      return
    }
    setError(null)
    setIsSubmitting(true)
    try {
      const data = await fetchLicenseFromApi(trimmed)
      if (!data) {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#5b7895] dark:bg-slate-900">
      <div className="w-full max-w-sm p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-600 shadow-lg">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">
          Geben Sie Ihre Lizenznummer ein, um die Anwendung zu nutzen.
        </p>
