      ...
      {isOpen && (
        <div className="mt-2 space-y-2">
          ...
          {isLoading ? (
            ...
            <p className="text-xs text-slate-500">Noch keine Portal-Benutzer eingeladen.</p>
          ) : (
            <ul className="space-y-1">
              {portalUsers.map((pu) => (
                <li
                  key={pu.id}
                  className="flex items-center justify-between gap-2 text-sm bg-white rounded border border-slate-200 px-3 py-1.5"
                >
                  <div className="min-w-0">
                    <span className="text-slate-700 truncate block">{pu.email}</span>
