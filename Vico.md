...
- **Rollout & Deploy / Go-Live (Admin):** Seite **`/release-rollout`** – für ein **freigegebenes** Release optional **Production-Deploy** anstoßen, darunter **Go-Live für Mandanten** (**alle** oder **Auswahl**), Standardfall **ohne** SQL; Umsetzung u. a. **`admin/src/components/ReleaseGoLivePanel.tsx`**, **`assignPublishedReleaseToTenantIds`** in **`admin/src/lib/mandantenReleaseService.ts`**.
...
**Production-Deploy aus dem LP (2026-04-05):** Freigegebenes Release → Button in **`AppReleaseEditor`** und zentrale Seite **`/release-rollout`** („Rollout & Deploy“) → Supabase Edge **`trigger-github-deploy`** (JWT, nur **`profiles.role = admin`**) → **`workflow_dispatch`** auf **`.github/workflows/deploy-pages-from-release.yml`** (Checkout per Tag/SHA aus **`ci_metadata`** bzw. Fallback **`Kanal/Version`**). Build + **`wrangler pages deploy`**; GitHub-Repo-Secrets für **CF** und **VITE_*** pro App. Audit-Action **`release.deploy_triggered`**. **Auf derselben Seite:** **Go-Live** für Mandanten (**`ReleaseGoLivePanel`**, Bulk **`assignPublishedReleaseToTenantIds`**). **Phase 2 (Assistent):** Rollout-Assistent (**`RolloutChecklistModal`**) mit Schritten Freigabe → Notes → Deploy → GitHub → Incoming → Go-Live; gemeinsame Deploy-Logik **`useReleaseDeployTrigger`** / **`ReleaseDeployPanel`**. Geplanter Ausbau: zentrale Env-Quelle (JSON/ Skript, Fragerunde 2).

**Stand Code (2026-04-10):** Lizenzportal **`supabase-license-portal.sql`** (Abschnitt 7: `app_releases`, Zuweisungen, Audit, RLS, `is_test_mandant`, `can_manage_app_releases`), Admin-UI **`/app-releases`**, **`/release-rollout`** (**`ReleaseDeployPanel`** + **`ReleaseGoLivePanel`**), **`MandantForm`** (Testmandant + Zuweisungen), Lizenz-API **Supabase Edge** **`supabase-license-portal/supabase/functions/license`** (`mandantenReleases` inkl. **`releaseAssignmentUpdatedAt`** + Overlay `appVersions` pro Kanal; Parität mit `admin/netlify/functions/license.ts`). **Deploy:** Edge **`trigger-github-deploy`**, GitHub **`deploy-pages-from-release.yml`**. Nach Zuweisung/Rollback: **`bumpClientConfigVersionsForTenantLicenses`** (alle Lizenzen des Mandanten). Mandanten-Apps: Parsing in **`licensePortalApi`** / **`fetchDesignFromLicense`**, **`LicenseContext`** / Portal-**`DesignContext`**, Banner **`shared/MandantenIncomingReleaseBanner.tsx`**, Hinweis bei geänderter Zuweisung **`shared/MandantenReleaseRolloutRefreshBanner.tsx`**, Pflicht-Reload bei **`active.forceHardReload`**: **`shared/MandantenReleaseHardReloadGate.tsx`** + **`shared/mandantenReleaseReloadBridge.ts`** (Haupt-App: **`PwaUpdatePrompt`** / `registerSW(true)`). **Offen/Feinschliff:** **WP-REL-03** (strukturierte Notes/CI-Tags).
