...
- **Update-Assistent / Rollout (Admin):** Seite **`/release-rollout`** – Modus **Update** oder **Rollback**; **Mehrkanal** (feste Reihenfolge Haupt-App → Kundenportal → Arbeitszeitenportal); **Mandanten-Umfang** alle / Auswahl / nur Testmandanten (Zusatzbestätigung ab 10 Mandanten bei „alle“); **Update:** je Kanal ein **freigegebenes** Release, **Incoming**-Übersicht (Lesen) mit Link zum Editor, **Deploy-Bestätigung (D4)** vor Go-Live, **`ReleaseDeployPanel`** pro Kanal, **Go-Live** mit **Live-Protokoll** (Zuweisung nacheinander pro Mandant×Kanal über **`setTenantChannelActiveRelease`**), **Abbrechen**, **fehlgeschlagene wiederholen**, Export **TSV/CSV**; **Rollback:** **Vorschau-Tabelle** (aktuell / Rollback-Ziel) mit Pagination, gleiches Protokoll. Nachvollziehbarkeit: **`/release-audit`** (Filter). Konzept: **`docs/Concept-Lizenzportal-Update-Assistent.md`**. Code u. a. **`RolloutTenantScope`**, **`RolloutUpdateRunBlock`**, **`ReleaseRollbackBulkPanel`**, **`RolloutLiveProtocol`**, **`rolloutLiveLog.ts`**, **`mandantenReleaseService.ts`** (inkl. **`fetchAppReleasesMetaByIds`**), **`tenantService`** mit **`is_test_mandant`** in der Mandantenliste. Bulk weiterhin **`assignPublishedReleaseToTenantIds`** nutzbar (Service); Einzelfall in **`MandantForm`** → **`MandantReleaseAssignmentsSection`** (Hinweis auf Assistent).
...
### 11.20 Mandanten-App-Releases: Lizenzportal-Verwaltung, Incoming, gestaffeltes Update (Stand 2026-04-10)
...
**Production-Deploy aus dem LP:** Freigegebenes Release → Button in **`AppReleaseEditor`** und **Update-Assistent** **`/release-rollout`** → Supabase Edge **`trigger-github-deploy`** (JWT, nur **`profiles.role = admin`**) → **`workflow_dispatch`** auf **`.github/workflows/deploy-pages-from-release.yml`** (Checkout per Tag/SHA aus **`ci_metadata`** bzw. Fallback Kanal/Version). Build + **`wrangler pages deploy`**; GitHub-Repo-Secrets für **CF** und **VITE_*** pro App. Audit-Action **`release.deploy_triggered`**. Im Assistenten: **`ReleaseDeployPanel`** und **`useReleaseDeployTrigger`** **je gewähltem Kanal**; optional **`RolloutChecklistModal`** (Checkliste, nicht vollständig in Wizard-Schritte überführt). Geplanter Ausbau: zentrale Env-Quelle (JSON/Skript).

**Stand Code (2026-04-10):** Lizenzportal **`supabase-license-portal.sql`** (Abschnitt 7: `app_releases`, Zuweisungen, Audit, RLS, `is_test_mandant`, `can_manage_app_releases`), Admin-UI **`/app-releases`**, **`/release-rollout`** (**Update-Assistent** mit Mehrkanal-Update, Live-Protokoll, Rollback-Vorschau; Komponenten siehe **§3** *Lizenzportal*), **`/release-audit`**, **`MandantForm`** (Testmandant; **`MandantReleaseAssignmentsSection`** für Einzelfälle mit Verweis auf Assistent). **`ReleaseGoLivePanel`** entfällt (ersetzt durch Assistent). Lizenz-API **Supabase Edge** **`supabase-license-portal/supabase/functions/license`**: `mandantenReleases` inkl. **`releaseAssignmentUpdatedAt`** + Overlay `appVersions` pro Kanal; **Incoming** nur für Pilotenliste und **Testmandanten**, nicht global über **`incoming_all_mandanten`** (Parität **`admin/netlify/functions/license.ts`**). **Deploy:** Edge **`trigger-github-deploy`**, GitHub **`deploy-pages-from-release.yml`**. Nach Zuweisung/Rollback: **`bumpClientConfigVersionsForTenantLicenses`**. Mandanten-Apps: **`licensePortalApi`** / **`fetchDesignFromLicense`**, **`LicenseContext`** / Portal-**`DesignContext`**, **`shared/MandantenIncomingReleaseBanner.tsx`**, **`shared/MandantenReleaseRolloutRefreshBanner.tsx`**, **`shared/MandantenReleaseHardReloadGate.tsx`** + **`shared/mandantenReleaseReloadBridge.ts`**. Konzept Assistent: **`docs/Concept-Lizenzportal-Update-Assistent.md`**. **Offen/Feinschliff:** **WP-REL-03** (strukturierte Notes/CI-Tags); optional Audit-Filter nach Mandanten-/Release-ID; **D13** Browser-Benachrichtigung nach Bulk-Lauf (Ausbau).
