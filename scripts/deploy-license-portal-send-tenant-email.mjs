#!/usr/bin/env node
/**
 * Deployt die Edge Function **send-tenant-email** im Lizenzportal-Supabase (zentraler Mandanten-Mailversand).
 *
 * Ausführung aus dem Repo-Root:
 *   npm run lp:deploy:send-tenant-email
 *   npm run lp:deploy:send-tenant-email -- --project-ref <ref>
 *
...
if (fs.existsSync(localSupabase)) {
  result = run(localSupabase, ['functions', 'deploy', 'send-tenant-email', ...extraArgs])
} else {
  console.warn('Hinweis: Lokales supabase-CLI nicht unter node_modules/.bin – nutze npx.')
  result = run('npx', ['--yes', 'supabase', 'functions', 'deploy', 'send-tenant-email', ...extraArgs])
}
