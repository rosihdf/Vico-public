#!/usr/bin/env node
/**
 * Deployt die Edge Function **send-tenant-email-server** im Lizenzportal (Cron / Shared Secret).
 *
 *   npm run lp:deploy:send-tenant-email-server
 *   npm run lp:deploy:send-tenant-email-server -- --project-ref <ref>
 */
...
if (fs.existsSync(localSupabase)) {
  result = run(localSupabase, ['functions', 'deploy', 'send-tenant-email-server', ...extraArgs])
} else {
  console.warn('Hinweis: Lokales supabase-CLI nicht unter node_modules/.bin – nutze npx.')
  result = run('npx', ['--yes', 'supabase', 'functions', 'deploy', 'send-tenant-email-server', ...extraArgs])
}
