  ...
  return (
    <div className="p-4">
      <div className="flex flex-wrap items-center gap-2 mb-2">
        <h2 className="text-xl font-bold text-slate-800">Standortabfrage</h2>
      ...
      <p className="text-xs text-amber-900 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-4 dark:bg-amber-950/30 dark:text-amber-100 dark:border-amber-800">
        ...
        <code className="text-[0.8rem]">docs/Noch-zu-erledigen.md</code> §3a abarbeiten; Lizenz-Feature{' '}
      ...
      {loading ? (
        ...
        <p className="text-slate-600">Keine Mitarbeiter mit Einwilligung vorhanden.</p>
      ) : (
          ...
          {locations.map((loc) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-medium text-slate-800">{getDisplayName(loc)}</p>
                  {loc.email && (
                    <p className="text-sm text-slate-500">{loc.email}</p>
                  )}
                  {hasLocation(loc) ? (
                    <>
                      <p className="text-sm text-slate-600 mt-1">
                        ...
                        {loc.accuracy != null && (
                          <span className="text-slate-500"> (ca. {Math.round(loc.accuracy)} m)</span>
                        )}
                      </p>
      ...
      <button
        ...
        onClick={load}
        disabled={loading}
        className="mt-4 px-4 py-2 rounded-lg text-sm font-medium border border-slate-300 text-slate-700 hover:bg-slate-50 disabled:opacity-50"
