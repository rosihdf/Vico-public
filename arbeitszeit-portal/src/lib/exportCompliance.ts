...
import { layoutForBriefbogenDin, layoutForBriefbogenDinFollowPage } from '../../../shared/pdfBriefbogenLayout'
...
export const exportZollPdf = async (
  ...
  const [letterheadPages, textLayout] = await Promise.all([
    fetchBriefbogenLetterheadPagesForPdf(supabase),
    fetchBriefbogenPdfTextLayout(supabase),
  ])
  const contentMargins = textLayout.margins
  const followCompactTop = textLayout.followPageCompactTop
  const doc = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' })
  const pageW = doc.internal.pageSize.getWidth()
  const pageH = doc.internal.pageSize.getHeight()
  const boxLh = letterheadPages ? layoutForBriefbogenDin(pageW, pageH, contentMargins) : null
  const yStartAfterBreak =
    letterheadPages && followCompactTop
      ? layoutForBriefbogenDinFollowPage(pageW, pageH, contentMargins).yStart
      : boxLh?.yStart ?? 0
  ...
  const colX = (i: number, left: number) => left + colWidths.slice(0, i).reduce((a, b) => a + b, 0)

  if (boxLh) {
    let y = boxLh.yStart
    const L = boxLh.left
    doc.text(`Zeiterfassung für Zoll-/Mindestlohnprüfung (MiLoG § 17)`, L, y)
    y += 6
    doc.text(`Mitarbeiter: ${employeeName} | Zeitraum: ${fromDate} bis ${toDate}`, L, y)
    y += 6
    doc.text(`Erstellt: ${new Date().toLocaleDateString('de-DE')}`, L, y)
    y += 8

    doc.setFont('helvetica', 'bold')
    headers.forEach((h, i) => {
      doc.text(h, colX(i, L), y)
    })
    y += 7
    ...
    for (const e of entries) {
      ...
      row.forEach((cell, i) => {
        doc.text(String(cell).slice(0, 25), colX(i, L), y)
      })
      y += 6
    }

    doc.text(
      'Aufbewahrung: Mindestens 2 Jahre (ArbZG § 16, MiLoG § 17), empfohlen 8 Jahre (Steuerrecht).',
      L,
      boxLh.yMax - 4
    )
  } else {
    ...
    for (const e of entries) {
      ...
      const row = [e.date, startStr, endStr, String(breakMin), String(workMin), employeeName]
      row.forEach((cell, i) => {
        doc.text(String(cell).slice(0, 25), colX(i, 14), y)
      })
      y += 6
    }
...
export const exportUrlaubsbescheinigungPdf = async (
  ...
  if (boxLh) {
    const L = boxLh.left
    const y0 = boxLh.yStart
    doc.setFontSize(14)
    doc.text('Urlaubsbescheinigung', pageW / 2, y0 + 6, { align: 'center' })
    doc.setFontSize(10)
    doc.text(`gemäß § 6 Abs. 2 Bundesurlaubsgesetz (BUrlG)`, pageW / 2, y0 + 13, { align: 'center' })
    let y = y0 + 24
    doc.text(`Hiermit wird bestätigt, dass im Kalenderjahr ${year}`, L, y)
    y += 7
    doc.text(`folgende Urlaubstage gewährt bzw. abgegolten wurden:`, L, y)
    y += 10
    doc.text(`Mitarbeiter/in: ${employeeName}`, L, y)
    y += 8
    doc.text(`Urlaubsanspruch ${year}: ${entitlementDays ?? '–'} Tage`, L, y)
    y += 8
    doc.text(`Gewährte/abgegoltene Tage: ${totalDays.toFixed(1)}`, L, y)
    y += 10
    doc.text('Einzelne Zeiträume:', L, y)
    y += 8
    for (const l of approvedLeaves) {
      if (y > boxLh.yMax - 28) break
      doc.text(
    ...
    doc.text(`Ort, Datum: _________________________`, L, boxLh.yMax - 12)
    doc.text(`Unterschrift Arbeitgeber/Vorgesetzter`, L, boxLh.yMax - 4)
  } else {
    doc.setFontSize(14)
