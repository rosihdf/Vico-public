...
/** Lädt Favicon, skaliert (quadratisch, max Kante), konvertiert zu WebP und lädt in Storage. */
export const uploadTenantFaviconWebP = async (
  tenantId: string,
  file: File
): Promise<{ publicUrl: string | null; error: string | null }> => {
  if (!tenantId?.trim()) return { publicUrl: null, error: 'Keine Mandanten-ID.' }
  ...
  const path = `${tenantId}/favicon.webp`

  try {
    const bitmap = await createImageBitmap(file)
    const w = bitmap.width
    const h = bitmap.height
    const ratio = Math.min(1, MAX_FAVICON_EDGE / Math.max(w, h))
...
export const removeTenantFaviconFromStorage = async (tenantId: string): Promise<{ error: string | null }> => {
  const path = `${tenantId}/favicon.webp`
  const { error } = await supabase.storage.from(BUCKET).remove([path])
  return { error: error?.message ?? null }
}
